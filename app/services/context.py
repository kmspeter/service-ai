...
class ContextBudgetManager:
    ...
    def __init__(
        ...
        *,
        ...
        if not self._fits_answer(None, (), ""):
            raise ContextBudgetError
        if not self._fits_summary_prompt(None, ()):
            raise ContextBudgetError
    ...
    async def prepare_conversation(
        ...
        *,
        ...
        """Keep the newest messages and summarize the prefix that must be removed."""
        ...
        split_at = max(0, len(valid_messages) - self.max_recent_messages)
    ...
    async def _summarize(
        ...
        *,
        ...
        """Summarize bounded batches so even one very long old message cannot overflow."""
    ...
    def _conversation_fits(
        ...
        return (
            self.token_counter.count(rewrite_prompt) <= self.available_input_tokens
    ...
    def _fits_answer(
        ...
        return (
            self.token_counter.count(self._answer_prompt(summary, messages, question, "[]"))
    ...
    def _fits_summary_prompt(
        ...
        return (
            ...
            <= self.context_window - self.summary_max_output_tokens
        )
    ...
    def _truncate_for_summary_prompt(self, summary: str | None) -> str | None:
        ...
        while low <= high:
            ...
            if self._fits_summary_prompt(candidate or None, ()):
                best = candidate
                low = midpoint + 1
            else:
                high = midpoint - 1
    ...
    def _truncate_summary_to_fit(
        ...
        *,
        ...
        while low <= high:
            midpoint = (low + high) // 2
            candidate = summary[-midpoint:] if midpoint else ""
            if self._conversation_fits(
                candidate or None, recent_messages, current_question
            ):
                best = candidate
                low = midpoint + 1
            else:
